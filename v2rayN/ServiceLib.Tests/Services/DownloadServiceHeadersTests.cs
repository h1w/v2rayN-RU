...
public class DownloadServiceHeadersTests
{
    ...
    public async Task TryDownloadString_ShouldSendCustomHeadersThroughBothDownloaders(bool useProxy, bool failFirstRequest)
    {
        ...
        foreach (var request in server.Requests)
        {
            ...
            request["User-Agent"].Should().Be("CustomSubscriptionClient/1.0");
            request["Authorization"].Should().Be("Bearer test-token");
            request["X-hwid"].Should().Be("test-device");
            request["Cookie"].Should().Be("session=test");
    ...
    public async Task TryDownloadString_ShouldKeepDefaultAcceptUserAgentAndBasicAuth(bool failFirstRequest)
    {
        ...
        foreach (var request in server.Requests)
        {
            ...
            request["User-Agent"].Should().Be("ExistingClient/1.0");
            request["Authorization"].Should().Be("Basic dXNlcjpwYXNzd29yZA==");
            request.ContainsKey("X-hwid").Should().BeFalse();
        }
    }
    ...
    public async Task TryDownloadString_ShouldNotShareHeadersWithOtherDownloads()
    {
        ...
        requests[0]["X-hwid"].Should().Be("first-device");
        requests[1].ContainsKey("X-hwid").Should().BeFalse();
        requests[1].ContainsKey("Accept").Should().BeFalse();
    }
